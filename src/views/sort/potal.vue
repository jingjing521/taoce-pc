            ...
            <el-container style="height:auto; border: 1px solid #eee" v-loading="loading">
              <el-aside width="200px">
                <el-menu unique-opened>
                  <el-submenu
                    v-for="(item,index) in shopCategoryList"
                    :key="index"
                    :index="index.toString()"
                  >
                    <template slot="title">{{item.cat_one_name}}</template>
                    <el-menu-item
                      v-for="(ite,idx) in item.twolist"
                      :key="idx"
                      @click="getGoods(item.cate_one,ite.cate_two)"
                    >{{ite.cate_two_name}}</el-menu-item>
                  </el-submenu>
                ...
                <el-row :gutter="10" style="padding:10px;width:100%">
                  <el-col :span="5" v-for="(item,index) in goodsList" :key="index">
                    <div class="text-center" @click="goDetail(item.goods_id,item.shopid)">
                      ...
                      <div
                        class="text-df"
                        style="overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; min-height: 44px;"
                      >{{item.goods_name}}</div>
                    </div>
                  </el-col>
                </el-row>
