  ...
  <div class="main_width stepBox" id="step">
    <el-form :model="param" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      ...
      <el-card class="box-card margin-bottom">
        ...
        <div>
          <el-table  :data="goodsDetail.multipleSelection" style="width: 100%" border  ref="multipleTable" >
      ...
      <el-card class="box-card margin-bottom">
        ...
        <div>
          <el-button
            ...
            @click="getClient"
          >选择委托客户+</el-button>请从用户身份认证信息中选择，若您未进行身份认证，请点击进行
          <router-link to="/authentication" target="_blank">
        ...
        <div>
          <ul class="shqrm_c">
            ...
            >
              ...
              <el-card
                :class=" !moreAddress ? 'box-card moreAddress':'box-card'"
                shadow="never"
                id="address-box"
              ...
              >
                <div v-for="item in addressList" :key="item.id" class="text item">
                  <el-radio
                    ...
                    :label="item.id"
                    @change="opt"
                  >{{ item.proAddress + item.address }}</el-radio>
                </div>
              </el-card>
              <div style="margin-top: 15px;">
                ...
                <el-button
                  ...
                  @click="moreAddress = !moreAddress"
                >{{ !moreAddress ? '更多地址':'收起地址'}}</el-button>
              </div>
            </div>
          </ul>
        </div>
...
export default {
  ...
  created() {
    ...
    if(window.localStorage.getItem("paoce_token-detail")){
      ...
      goodsDetail.multipleSelection.forEach(function(v,i){ 
        _this.param.goodsAddtionalItemList.push({fwid:v.additionalServiceEntity.id,jcsl:v.additionalServiceEntity.numItem})
      })
